...
import {
  ArrowLeft, Check, ExternalLink, FileSpreadsheet,
  FileText, FolderOpen, Pencil, UserRound, X,
} from "lucide-react";
...
export default function CaseWorkspace() {
  ...
  if (!c || c.productType === "Unknown") {
    return <div className="fb-card p-10 text-center text-slate-500">No processed case selected.</div>;
  }

  const readyToGenerate = c.fields.length > 0 && c.fields.every((field) => field.value.trim().length > 0);
  const manualRequiredCount = c.fields.filter((field) => field.manualRequired && !field.value.trim()).length;
  const generateLaunchDocuments = () => {
  ...
  return (
    <div className="animate-fade-up">
      <PageHeader
        title="All key terms have been extracted"
      ...
      <div className="flex flex-col gap-5">
            ...
            <div className="border-b border-slate-200 bg-white px-5 py-4">
              <div className="flex min-w-0 items-center gap-3">
                ...
                {manualRequiredCount > 0 && (
                  <span className="shrink-0 whitespace-nowrap rounded-full border border-amber-200 bg-amber-50 px-3 py-1.5 text-[10px] font-extrabold leading-none text-amber-700">
                    {manualRequiredCount} manual {manualRequiredCount === 1 ? "determination" : "determinations"} required
                  </span>
                )}
        ...
        <section className="fb-card p-5">
          ...
          <p className="mt-1 text-[12px] text-slate-500">Both outputs are generated together from the same Trader-confirmed data.</p>
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <div className="flex items-center gap-3 rounded-xl border border-demo-line p-3"><div className="flex h-9 w-9 items-center justify-center rounded-lg bg-demo-light text-demo-blue"><FileText size={18} /></div><div><div className="text-[13px] font-bold text-demo-dark">Launch Documents</div><div className="text-[11.5px] text-slate-500">{c.documents.length} files generated for Trader review</div></div></div>
          ...
          <div className="mt-5 flex justify-center">
            <button className="fb-btn-primary !px-12 !py-3.5 !text-[15.5px]" disabled={!readyToGenerate} onClick={generateLaunchDocuments}>
              <FileText size={17} /> Generate Launch Documents &amp; Product Code
            </button>
          </div>
          {manualRequiredCount > 0 && (
            <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-[11.5px] font-semibold text-amber-800">
              Complete the highlighted manual fields before generating the outputs.
            </div>
          )}
        </section>
      </div>
